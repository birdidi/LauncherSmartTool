...
public class LayoutProfileMi {
    ...
    public static List<CellBean> layout() {
        ...
        //第一屏
        list.add(CellBean.build(0, 0, 0, 3, "天气", CellBean.CANDIDATENAME_WEATHER, RecommandAppPool.TYPE_WEATHER));
        list.add(CellBean.build(0, 0, 1, 3, CellBean.TYPE_APP, "图库", CellBean.CANDIDATENAME_GALLERY));
        list.add(CellBean.build(0, 0, 2, 3, "音乐", CellBean.CANDIDATENAME_MUSIC, RecommandAppPool.TYPE_MUSIC));
        list.add(CellBean.build(0, 0, 3, 3, "个性主题", CellBean.CANDIDATENAME_THEME, RecommandAppPool.TYPE_THEME));
        list.add(CellBean.build(0, 0, 0, 4, CellBean.TYPE_APP, "设置", CellBean.CANDIDATENAME_SETTING));
        list.add(CellBean.build(0, 0, 1, 4, "应用商店", RecommandAppPool.TYPE_APP_STORE));
        list.add(CellBean.build(0, 0, 2, 4, "手机管家", RecommandAppPool.getFuzzyRegexs("安全中心"), RecommandAppPool.TYPE_SAFE));
        ...
        list.add(CellBean.build(0, 1, 1, 0, "阅读", RecommandAppPool.getFuzzyRegexs("阅读"), RecommandAppPool.TYPE_I_READER));
        list.add(CellBean.build(0, 1, 2, 0, CellBean.TYPE_APP, "文件管理"));
        list.add(CellBean.build(0, 1, 3, 0, CellBean.TYPE_APP, "商城", RecommandAppPool.getFuzzyRegexs("小米商城", "商城")));
        list.add(CellBean.build(0, 1, 0, 1, CellBean.TYPE_APP, "游戏", RecommandAppPool.getFuzzyRegexs("游戏", "游戏")));
        list.add(CellBean.build(0, 1, 1, 1, "视频", CellBean.CANDIDATENAME_VIDEO, RecommandAppPool.TYPE_VIDEO));
        list.add(CellBean.build(0, 1, 2, 1, CellBean.TYPE_APP, "高德地图", RecommandAppPool.GAODE));
        list.add(CellBean.build(0, 1, 3, 1, "资讯", RecommandAppPool.getFuzzyRegexs("资讯"), RecommandAppPool.TYPE_NEWS));
        ...
        commons.add(CellBean.build(RecommandAppPool.GAODE));
        list.add(CellBean.build(0, 1, 0, 2, "常用", commons));
        ...
        list.add(CellBean.build(0, 1, 1, 2, "娱乐", yule));
        ...
        list.add(CellBean.build(0, 1, 2, 2, "推荐", recomment));

        //DOCK
        list.add(CellBean.build(1, 0, 0, 0, "电话", BaseThemeData.INTENT_PHONE));
        list.add(CellBean.build(1, 0, 1, 0, "短信", BaseThemeData.INTENT_MMS));
